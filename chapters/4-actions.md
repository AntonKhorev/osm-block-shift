...
В качестве редактора мы выберем, конечно же, [JOSM](https://wiki.openstreetmap.org/wiki/RU:JOSM). Подходящих альтернатив у него просто нет. Многие, пытающиеся описанным выше способом исправлять уже введённые данные, делают это из [iD](https://wiki.openstreetmap.org/wiki/RU:ID). Из iD сделать правильно то, что мы хотим, практически нереально. Более того, этот редактор вообще не подходит для модификации существующей геометрии. Поскольку данный редактор находится [в одном клике](https://www.openstreetmap.org/edit) от просмотра карты на сайте, он считается редактором для новичков. На самом деле iD является редактором для добавления и незначительного изменения данных в тех случаях, когда на компьютере не установлен более подходящий редактор. Новичкам пользоваться iD для исправления существующей геометрии не следует, так как результат может оказаться противоположным. Из iD достаточно легко, легче, чем из JOSM, вносить новые ошибки, в частности из-за того, сколько различных действий закреплено за левой кнопкой мыши. Впрочем, в JOSM их тоже закреплено немало, что нам предстоит увидеть.

Из прочих редакторов нам по очевидным причинам не подойдут те, которые предназначены для редактирования [POI](https://wiki.openstreetmap.org/wiki/RU:Точки_интереса), например, [OsmAnd](https://wiki.openstreetmap.org/wiki/RU:OsmAnd) и [MAPS.ME](https://wiki.openstreetmap.org/wiki/MAPS.ME) со [своими производными](https://wiki.openstreetmap.org/wiki/MAPS.ME#Competitors). Раз уж речь зашла о мобильных редакторах, то придётся признать неподходящими их все. Хоть какие-то шансы есть у [Vespucci](https://wiki.openstreetmap.org/wiki/RU:Vespucci), так как он предназначен для редактирования геометрии. Предназначен он для этого, правда, относительно своей платформы, так как из iD на компьютере редактировать было бы легче. На мобильных же устройствах вариантов лучше нет.
...
Поскольку действия из JOSM мы будем рассматривать на протяжении оставшейся части записи, договоримся об их обозначениях. Для краткости мы будем называть режимы и команды по клавишам, на которые надо нажать, чтобы их выполнить. Например, команду выстраивания точек по прямой мы будем называть командой <kbd>L</kbd>. Так как многие из команд придётся использовать довольно часто, клавиши для их вызова имеет смысл запомнить, чтобы не искать соответствующих пунктов меню и вообще не отодвигать мышиный курсор от редактируемого места. Некоторые из режимов требуют для активации повторного нажатия клавиши, и их мы будем обозначать двойной буквой. Например, режим лассо мы будем называть режимом <kbd>SS</kbd>.
...
[^4-x]: В последних версиях JOSM это произошло с режимом <kbd>X</kbd>, который мы используем по неосновной операции. Хотя разработчики могут и клавишу поменять, что произошло несколько лет назад с режимом удаления, но мы этот режим рассматривать не будем.
...
evaluateManualShift --> |удовлетворительно| deleteTempLines[удалить временные линии]
evaluateManualShift --> |неудовлетворительно| cancelManualShift[отменить сдвиг вручную]
cancelManualShift --> drawGuides[нарисовать направляющую рамку]
estimateOverlap --> |серьёзные| determineStraightSections[определить прямые участки улиц, симметрию тротуаров, ...]
determineStraightSections --> drawAxes[провести оси и вспомогательные линии]
drawAxes --> alignExistingGeometry[выровнять по ним существующую геометрию]
alignExistingGeometry --> drawGuides
drawGuides --> restoreCorners[восстановить углы]
restoreCorners --> transform[Transform4]
transform --> deleteTempLines
deleteTempLines --> fixAfterShift[выполнить поправки после сдвига]
```
